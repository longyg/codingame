...
void intersect(vec2 c1, float r1_turns, vec2 c2, float r2_turns, std::set<vec2> *inter)
{
    ...
    if (inter1.x > 0 && inter1.y > 0 && inter1.x < map_size.x && inter1.y < map_size.y)
        inter->insert(inter1);
    if (inter2.x > 0 && inter2.y > 0 && inter2.x < map_size.x && inter2.y < map_size.y)
        inter->insert(inter2);
    ...
    bool add_objective(objective &obj)
    {
        ...
        for (auto &di : obj.candidates)
        {
            ...
            if (!contracts[di].empty())
            {
                ...
                for (auto i = inter_others.begin(); i != inter_others.end(); i++)
            ...
            if (contracts[di].empty())
            {
                ...
                if (tmp_inter_set[di].empty())
                    for (float i = 0; i < 6; i += 1)
                        tmp_inter_set[di].insert(d.pos + vec2({int(100 * std::cos(2 * M_PI * i / 6)), int(100 * std::sin(2 * M_PI * i / 6))}));
        ...
        for (auto &p : players)
        {
            ...
            for (auto &d : p.drones)
            {
                ...
                for (int i = 0; i < zones.size(); i++)
                    if (norm(d.speed) > 70 && abs(det(zones[i].pos - d.pos, d.speed / norm(d.speed))) < 100 && dot(zones[i].pos - d.pos, d.speed) > 0)
                    {
                        int dist = d.turns2(zones[i]);
                        if (d.expected_dest != nullptr && d.turns2dest < dist)
                            continue;
                        if (d.expected_dest != nullptr)
            ...
            for (int di = 0; di < n_drones - 1; di++)
            {
                ...
                if (is_my_zone)
                {
                    ...
                    for (int mydi = di; mydi <= di; mydi++)
                    {
                        ...
                        while (mydi + 1 < n_drones && players[my_id].drones[mydi].turns2(z) == players[my_id].drones[mydi + 1].turns2(z))
                            mydi++;
                        int diff = attackers[di][0].turns2(z) - players[my_id].drones[mydi].turns2(z);
                        objective obj;
